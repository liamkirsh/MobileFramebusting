...
output = open("no_xfo_yes_fa.csv", "w")
output.write("# domain,desktop_has_only_fa,mobile_has_only_fa\n")
    ...
    with open(elem, 'r') as csvfile:
        ...
        for row in reader:
            ...
            if desktop_xfo.strip() == "TIMEOUT_ERROR":
                ...
                d_url = 1 if not is_valid_xfo(desktop_xfo) and is_valid_csp(desktop_csp) else 0
            m_url = 1 if not is_valid_xfo(mobile_xfo) and is_valid_csp(mobile_csp) else 0
